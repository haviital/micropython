// Note: this is still preliminary for Pokitto. Must be cleaned up.
...
struct gpio_t;
#define mp_hal_pin_obj_t const struct gpio_t*
#define mp_hal_pin_high(p) ((*((p)->reg_set)) = (p)->mask)
#define mp_hal_pin_low(p)  ((*((p)->reg_clr)) = (p)->mask)
#define mp_hal_pin_read(p) (((*((p)->reg_in)) & (p)->mask) ? 1 : 0)
